...
class NWAlignerModel(pl.LightningModule):

    def __init__(self, args):
        super(NWAlignerModel, self).__init__()
...
class HMM3AlignerModel(NWAlignerModel):

    def __init__(self, args):
        super(HMM3AlignerModel, self).__init__(args)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Obtain alignment statistics + visualizations
        gen = self.aligner.traceback(seq, order)
        # TODO fix validation stats, there is an infinite loop somewhere here.
        # statistics = self.validation_stats(
        #     x, y, xlen, ylen, gen, s, A, predA, theta, gap, batch_idx)
        # statistics = pd.DataFrame(
        #     statistics, columns=[
    ...
    # TODO: uncomment once validation_stats is working
    # def validation_epoch_end(self, outputs):
    #     loss_f = lambda x: x['validation_loss']
    #     losses = list(map(loss_f, outputs))
    #     loss = sum(losses) / len(losses)

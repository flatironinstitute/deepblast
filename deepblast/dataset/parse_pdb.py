...
def readPDB(filename, verbose=False):
    ''' Reads a simple pdb file.  it only reads the first chain if
    there are more than one.  It extracts the sequence and Calpha
    records from it.  It does a little light reality checking to
    make sure things are consistent, but doesnt go through every
    possible common malfunctions one finds in a pdb file as this
    is just a lightweight reader. The return types include a
    ...
    with open(filename) as pdbfile:
        for line in pdbfile:
            ...
            if line[:4] == 'ATOM' or line[:6] == "HETATM":
                # Split the line
                rec = PDBATOM(line[:6], int(line[6:11]), line[12:16],
                              line[16], line[17:20], line[21],
                              int(line[22:26]),line[26],
                              float(line[30:38]), float(line[38:46]),
                              float(line[46:54]), line[54:60], line[60:66],
                ...
                if rec.atomname==' CA ':
                    ...
                    if rec.seqnum - last_res_num != 1:
                        raise ValueError(
                            "ERROR: {} missing or duplicate residue {}".format(
                                filename,rec.seqnum))
                        flag=False
                    ...
                    if rec.resname in aa321:
                        ...
                        h = 'Z'
                        warnings.warn(
                            "WARNING: unknown residue type ",
                            rec.resname," ",rec.seqnum
                        )
                        warnings.warn(
                            "%3d %1s %3s %8.3f %8.3f %8.3f\n"%(
                                int(rec.resnum), h, rec.resname,
                                rec.x, rec.y, rec.z))
                    if rec.resname=="XXX":
                        warnings.warn("warning: Fake residue XXX type detected at",
                                      rec.seqnum)
                    points.append([rec.x, rec.y, rec.z])
                    seq += h
    return PDBCA(seq, np.array(points), first_res_num, len(seq))

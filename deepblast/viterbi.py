...
def _forward_pass(theta, G, operator='softmax'):
    ...
    G : torch.Tensor
        Transition log probabilities of shape N x M x S x S.
    operator : str
    ...
    for i in range(1, N):
        for j in range(1, M):
            print('i', i, 'j', j, 'V',  V[i, j], Q[i, j])
            V[i, j, m], Q[i, j, m] = operator.max(
                theta[i - 1, j - 1, m] + torch.Tensor([
                    G[i - 1, j - 1, m, m] + V[i - 1, j - 1, m],
                    G[i - 1, j - 1, m, x] + V[i - 1, j - 1, x],
                    G[i - 1, j - 1, m, y] + V[i - 1, j - 1, y]
                ])
            )
            V[i, j, x], Q[i, j, x] = operator.max(
                theta[i - 1, j, x] + torch.Tensor([
                    G[i - 1, j, x, m] + V[i - 1, j, m],
                    G[i - 1, j, x, x] + V[i - 1, j, x],
                    G[i - 1, j, x, y] + V[i - 1, j, y]
                ])
            )
            V[i, j, y], Q[i, j, y] = operator.max(
                theta[i, j - 1, y] + torch.Tensor([
                    G[i, j - 1, y, m] + V[i, j - 1, m],
                    G[i, j - 1, y, x] + V[i, j - 1, x],
                    G[i, j - 1, y, y] + V[i, j - 1, y]
                ])
            )
